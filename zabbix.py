...
def metric():
    data = json.dumps(
    {
       ...
       "params":{
           "output":["hostid","name"],
           "groupids":"48",   #必须选择zabbix里群组里的graphite-groups组
       },
       "auth":auth_key(), 
       "id":1,
    })
...
class ZabbixSender:
        ...
        def getGraphiteData(self):
                self.metric_value=q.get(block=True)
                url="http://127.0.0.1/render/?target=%s&from=-10second&format=json"%self.metric_value
                s_time = time.time()
                try:
        ...
        def AddData(self,value, clock = None):
		host,key=self.metric_value.split('.')
...
if __name__ == '__main__':
    while 1:
        ...
        for graphite_values in sender.getGraphiteData(): 
		sender.AddData('%s'%graphite_values[0])
        res = sender.Send()
